...
int DMXDriver::getDeviceConnected()
{
	return device_connected;
}
...
* Parameters: none
**/
int DMXDriver::FTDI_ListDevices()
{
	FT_STATUS ftStatus;
	DWORD numDevs = 0;
...
**/
int DMXDriver::FTDI_SendData(int label, unsigned char *data, int length)
{
...
* Parameters: Label, Pointer to Data Structure, Length of Data
**/
int DMXDriver::FTDI_ReceiveData(int label, unsigned char *data, unsigned int expected_length)
{
...
* Parameters: none
**/
void DMXDriver::FTDI_PurgeBuffer()
{
...
**/
uint16_t DMXDriver::FTDI_OpenDevice(int device_num)
{
	int RTimeout = 120;
	int WTimeout = 100;
	int VersionMSB = 0;
...
// Read a DMX packet
uint8_t DMXDriver::FTDI_RxDMX(uint8_t label, unsigned char *data, uint32_t* expected_length)
{
